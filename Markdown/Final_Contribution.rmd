...
Given our simulated Data, we calculate the Probit model for the binary dependent variable y given x to estimate the Maximum Likelihood estimator $\hat { \beta } _ {j  }$:
$$
f \left( y _ { i } | \mathbf { x } _ { i } \right) = \left[ \Phi \left( \mathbf { x } _ { i } ^ { \prime } \mathbf { \beta } \right) \right] ^ { y _ { i } } \left[ 1 - \Phi \left( \mathbf { x } _ { i } ^ { \prime } \boldsymbol { \beta } \right) \right] ^ { \mathbf { - y } _ { i } } \quad y _ { i } = 0,1
$$
